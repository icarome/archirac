...
for (; row = states.rows[i]; i++) {
    ...
    if (row.classList.contains("cover")) {
        (function(id) {
            ...
            row.children[2].children[1].addEventListener('click', function() {
              const xhr = new XMLHttpRequest();
              xhr.open("POST", '/cover/' + id.substr(6) + '/stop', true);
              xhr.send();
          });
            row.children[2].children[2].addEventListener('click', function() {
                const xhr = new XMLHttpRequest();
    ...
    if (row.classList.contains("climate")) {
        (function(id) {
            row.children[2].children[0].addEventListener('click', function() {
                const xhr = new XMLHttpRequest();
                xhr.open("POST", '/climate/' + id.substr(8) + '/set?mode=cool', true);
                xhr.send();
            });
            row.children[2].children[1].addEventListener('click', function() {
              const xhr = new XMLHttpRequest();
              xhr.open("POST", '/climate/' + id.substr(8) + '/set?mode=off', true);
              xhr.send();
            });
            row.children[2].children[2].addEventListener('click', function() {
                const xhr = new XMLHttpRequest();
                xhr.open("POST", '/climate/' + id.substr(8) + '/set?targettemperature=%2B', true);
                xhr.send();
            });
            row.children[2].children[3].addEventListener('click', function() {
                const xhr = new XMLHttpRequest();
                xhr.open("POST", '/climate/' + id.substr(8) + '/set?targettemperature=-', true);
                xhr.send();
            });
        })(row.id);
    }
